...
func (node *BPNode) deleteItem(key int64) bool {
	...
	for i := 0; i < num; i++ {
		if node.Items[i].key > key {
			...
			//删除当前的节点
			copy(node.Items[i:], node.Items[i+1:])         //复制后面的节点到前面去
...
func (node *BPNode) deleteChild(child *BPNode) bool {
	...
	for i := 0; i < num; i++ {
		if node.Nodes[i] == child {
			copy(node.Nodes[i:], node.Nodes[i+1:])         //在父节点中删除这个孩子节点
...
func (t *BPTree) deleteItem(parent *BPNode, node *BPNode, key int64) {
	for i := 0; i < len(node.Nodes); i++ {
		if key <= node.Nodes[i].maxKey || i == len(node.Nodes)-1 { //遍历到了最后一个都没有，就进入到最后一个节点中
			t.deleteItem(node, node.Nodes[i], key) //低归的进入到这个函数中进行添加节点
			break
		}
	}
	if len(node.Nodes) < 1 {
...
func (t *BPTree) itemMoveOrMerge(parent *BPNode, node *BPNode) {
	...
	if left != nil && len(left.Items) > t.halfw {
	...
	//将右侧节点取一个记录到被删除的节点
	if right != nil && len(right.Items) > t.halfw {
...
func (t *BPTree) childMoveOrMerge(parent *BPNode, node *BPNode) {
	...
	//将左边的节点取一个记录移动到被删除的节点
	if left != nil && len(left.Nodes) > t.halfw {
		//左边的节点存在，同时删除一个元素不会影响到树的结构
	...
	//将右侧节点取一个记录到被删除的节点
	if right != nil && len(right.Nodes) > t.halfw {
		//右边节点取出一个，一定比被删除的节点要大，所以取出第一个
